...
const createElection = async (req, res) => {
  ...
  try {
    ...
    // Log dates in Cameroon time (local) and UTC format
    console.log(
      "Start Date (Local - Africa/Douala):",
      moment.tz(startDate, cameroonTimeZone).format()
    );
    console.log(
      "End Date (Local - Africa/Douala):",
      moment.tz(endDate, cameroonTimeZone).format()
    );
    console.log(
      "Start Date (UTC):",
      moment.unix(startTimestamp).utc().format()
    );
    console.log("End Date (UTC):", moment.unix(endTimestamp).utc().format());

    // Election data that will be sent to the blockchain
  ...
  try {
    ...
    for (let i = 0; i < electionCountValue; i++) {
      ...
      try {
        ...
        console.warn(
          `Could not fetch candidates for election ID: ${i}`,
    ...
    console.error(
      "Error fetching elections from the blockchain:",
      error.message
    );
    res.status(500).json({ error: error.message || "Server Error" });
  }
...
const getElectionById = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn(
        `Could not fetch candidates for election ID: ${electionId}`,
        candidateError.message
      );
    }
    ...
    console.error(
      "Error fetching election from the blockchain:",
      error.message
    );
    res.status(500).json({ error: error.message || "Server Error" });
  }
...
const updateElection = async (req, res) => {
  ...
  try {
    ...
    if (!res.headersSent) {
      res.status(500).json({
        error: error.message || "Failed to update election on blockchain",
      });
    }
  }
};
...
const deleteElection = async (req, res) => {
  ...
  try {
    ...
    if (currentTime < election.endDate) {
      return res
        .status(400)
        .json({ msg: "Election cannot be deleted before its end date" });
    }
    ...
    res.status(500).json({
      error: error.message || "Failed to delete election from blockchain",
    });
  }
};
...
const closeElection = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      error: error.message || "Failed to close election on blockchain",
    });
  }
};
